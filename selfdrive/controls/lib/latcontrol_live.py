...
class LatControlLive():
  def __init__(self, CP, OP=None):
    ...
    self.angle_ctrl = LatControlAngle(CP)
 ...
 #   self.lqr = LatControlLQR(CP)
  ...
  def _select_ctrl(self, ctrl_type):
    if ctrl_type == 'angle':
      self.ctrl_type = 'angle'
      self.ctrl = self.angle_ctrl
    elif ctrl_type == 'pid':
      self.ctrl_type = 'pid'
      self.ctrl = self.pid
    elif ctrl_type == 'indi':
...
#      self.ctrl = self.lqr
  ...
  def _update_control(self):
    if self.op_params.get(ENABLE_LAT_PARAMS):
      ...
      self._select_ctrl('angle' if self.CP.steerControlType == car.CarParams.SteerControlType.angle else self.CP.lateralTuning.which())
